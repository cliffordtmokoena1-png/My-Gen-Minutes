...
impl DbManager {
  pub fn new() -> Self {
    ...
    let ssl_opts = if cfg!(target_os = "linux") {
      SslOpts::default().with_root_cert_path(Some(Path::new("/etc/ssl/certs/ca-certificates.crt")))
      // FOR EC2:
      // SslOpts::default().with_root_cert_path(Some(Path::new("/etc/ssl/certs/ca-bundle.crt")))
    } else {
      SslOpts::default().with_root_cert_path(Some(Path::new("/etc/ssl/cert.pem")))
    };
