...
type UseRegenerateMinutesProps = {
  transcriptId: number;
  setMinutesManager: React.Dispatch<React.SetStateAction<MinutesState>>;
  onSuccess?: () => void;
};

export const useRegenerateMinutes = ({
  transcriptId,
  setMinutesManager,
  onSuccess,
}: UseRegenerateMinutesProps) => {
  ...
  const regenerateMinutes = async (feedback: string) => {
    ...
    try {
      ...
      // Refresh minutes data and stop streaming state

      await mutate(["/api/get-minutes", transcriptId], undefined, { revalidate: true });

      setMinutesManager((prev) => stopStreaming(prev));

      safeCapture("feedback_sent", {
        transcript_id: transcriptId,
        feedback,
      });
      ...
      toast({
        title: "Failed to regenerate minutes",
        description:
